...
export default function UserReview() {
  ...
  const deleteReview = async (id) => {
    const confirm = await Swal.fire({ title: "Delete Review?", text: "Pakka delete karna hai bhai?", icon: "warning", showCancelButton: true, confirmButtonText: "Delete", cancelButtonText: "Cancel" });
    if (!confirm.isConfirmed) return;
    try {
      await fetch(`${API}/api/reviews/delete/${id}`, { method: "DELETE" });
      Swal.fire("Deleted", "Review delete ho gaya.", "success");
      fetchReviews();
    } catch (err) {
  ...
  const submitReply = async (id) => {
    if (!replyText.trim()) return Swal.fire("Empty Reply", "Bhai kuch likho to!", "warning");
    try {
      await fetch(`${API}/api/reviews/update/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ adminReply: replyText }) });
              ...
              return (
                <div className="review-card" key={rev._id}>
                  ...
                  <div className="review-actions">
                    ...
                    <button className="review-btn review-btn-hide" onClick={() => toggleStatus(rev._id)}>
                      {currentStatus === "approved" ? "Hide" : "Show"}
                    </button>
                    <button className="review-btn review-btn-delete" onClick={() => deleteReview(rev._id)}>
                      Delete
                    </button>
                  </div>
                  {openReply === rev._id && (
