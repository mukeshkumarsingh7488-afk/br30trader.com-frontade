...
async function toggleVIP(id) {
  try {
    const res = await fetch(`${window.API_BASE_URL}/api/auth/update-vip/${id}`, {
      method: "PUT",
...
async function toggleBlock(id) {
  try {
    const res = await fetch(`${window.API_BASE_URL}/api/auth/block-user/${id}`, {
      method: "PUT",
      headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
    });
    const data = await res.json();
...
async function deleteUser(id) {
  if (!confirm("Bhai, pakka udaana hai? (Soche samjhe bina mat dabao!)")) return;
  try {
    const res = await fetch(`${window.API_BASE_URL}/api/auth/delete-user/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
    });
    const data = await res.json();
...
async function loadSalesData(startDate = "", endDate = "") {
  ...
  try {
    ...
    const data = await res.json();
    if (data.success) {
      document.getElementById("total-revenue").innerText = `₹${data.totalRevenue.toLocaleString("en-IN")}`;
      const counts = {};
      data.sales.forEach((sale) => {
        const title = sale.courseName || (sale.course ? sale.course.title : null);
      ...
      if (document.getElementById("best-course-name")) {
        document.getElementById("best-course-name").innerText = bestSeller.toUpperCase();
      }
