...
async function saveTokenToDatabase(token) {
  try {
    ...
    const userId = userData._id || userData.id || (userData.user && userData.user._id);
    if (!userId) {
      console.log("❌ User ID nahi mili! LocalStorage mein 'userData' check karo.");
...
function calculateRisk() {
  ...
  if (capital > 0 && riskPercent > 0 && entry > 0 && sl > 0) {
    ...
    document.getElementById("risk-amt").innerText = "₹" + totalRiskAmount.toLocaleString();
...
function submitDailyCheck() {
  const checkboxes = document.querySelectorAll('.tool-card input[type="checkbox"]');
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      submitBtn.style.background = "linear-gradient(135deg, #a020f0 0%, #6d28d9 100%)";
...
function calculateAnalytics(trades) {
  ...
  let net = gross - tax;
  document.getElementById("total-gross").innerText = `₹${gross.toLocaleString()}`;
...
function displayTrades(trades) {
  ...
  if (!trades || trades.length === 0) {
    list.innerHTML = "<p style='color: #94a3b8; text-align: center; padding: 20px;'>No trades recorded yet.</p>";
    return;
  }
...
async function fetchNotifications() {
  ...
  try {
    ...
    if (Array.isArray(notifs)) {
      ...
      if (list) {
        if (notifs.length === 0) {
          list.innerHTML = '<p style="padding:15px; color:gray; text-align:center;">Abhi koi alert nahi hai.</p>';
        } else {
...
async function loadNotifications() {
  ...
  try {
    ...
    if (!Array.isArray(notifications) || notifications.length === 0) {
      container.innerHTML = '<p style="padding:15px; color:gray; text-align:center;">Abhi koi alert nahi hai.</p>';
      return;
    }
    notifications.forEach((notif) => {
      const title = notif.senderName || "Admin Alert";
      const message = notif.message;
      const time = notif.date ? new Date(notif.date).toLocaleString() : "Recent";
      const html = `
...
if (clearBtn) {
  clearBtn.addEventListener("click", async () => {
    ...
    try {
      const res = await fetch(`${window.API_BASE_URL}/api/notifications/clear-all`, {
        method: "DELETE",
        headers: { "x-auth-token": token },
      });
      if (response.ok) {
...
const checkout = async function (courseId) {
  ...
  const token = localStorage.getItem("token");
  const couponCode = window.currentCoupon && window.currentCoupon.code ? window.currentCoupon.code : "";
...
async function fetchTraders() {
  try {
    ...
    if (user && user.isVip) {
      let myIndex = allTraders.findIndex((t) => String(t._id) === String(user._id));
      let myProfile;
      if (myIndex !== -1) {
    ...
    allTraders = allTraders.filter((v, i, a) => a.findIndex((t) => t._id === v._id) === i);
    displayNextBatch();
...
function appendTraderHTML(Trader, rank, delay) {
  ...
  let userPic = rawPath && rawPath.startsWith("http") ? rawPath : rawPath ? `${window.API_BASE_URL}/uploads/${rawPath.split(/[\\/]/).pop()}` : `https://ui-avatars.com/api/?name=${encodeURIComponent(Trader.name)}&background=111&color=00ff88&bold=true`;
  const row = `
        ...
        if (imgTag) {
          if (course.thumbnail && course.thumbnail.trim() !== "") {
            ...
            if (fullPath.includes("cloudinary")) {
              ...
              fullPath = fullPath.replace("/upload/", `/upload/f_auto,q_auto,${imgWidth}/`);
            }
            imgTag.src = fullPath;
  ...
  try {
    ...
    if (data.success && data.coupon) {
      ...
      boxes.forEach((box) => {
        ...
        const applyBtn = box.querySelector("button") || box.querySelector(".apply-discount-btn");
        if (applyBtn) {
        ...
        if (!expiryTag) {
          ...
          expiryTag.style = "color: #ff8e8e; font-size: 12.5px; margin-top: 8px; font-weight: 800; text-transform: uppercase; animation: blink-expiry 1.5s infinite; letter-spacing: 0.5px;";
      ...
      window.currentCoupon = null;
      document.querySelectorAll(".coupon-display-box").forEach((box) => (box.style.display = "none"));
    }
  } catch (e) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("click", async function (e) {
    ...
    if (target.classList.contains("apply-coupon-btn")) {
      if (!currentCoupon) return alert("Bhai, filhal koi coupon available nahi hai.");
      const card = target.closest(".poster-card");
      const priceElement = card.querySelector(".price");
      let originalPrice = parseInt(priceElement.innerText.replace(/[^\d]/g, ""));
    ...
    if (target.classList.contains("payBtn")) {
      ...
      const isCouponApplied = card.getAttribute("data-coupon-applied") === "true";
...
async function syncAdminData() {
  try {
    ...
    const courses = await res.json();
    courses.forEach((c) => {
      const targetBtn = document.querySelector(`.payBtn[data-course="${c.title}"]`);
      const targetPrice = document.querySelector(`.price[data-course="${c.title}"]`);
      if (targetBtn) {
