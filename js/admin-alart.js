...
async function sendAlert() {
  ...
  try {
    ...
    const pushResponse = await fetch(`${window.API_BASE_URL}/api/admin/send-all`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
document.getElementById("admin-title").addEventListener("keypress", function (event) {
  if (event.key === "Enter") {
    event.preventDefault();
    document.getElementById("admin-msg").focus();
  }
});
document.getElementById("admin-msg").addEventListener("keypress", function (event) {
  if (event.key === "Enter") {
    event.preventDefault();
    sendAlert();
  }
