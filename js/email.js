...
if (sendBulkBtn) {
  sendBulkBtn.addEventListener("click", async (e) => {
    ...
    statusMsg.innerHTML = '<span class="loader" style="color: #00ff88;">Database se users nikaal kar mail bhej raha hoon...</span>';
    try {
      const response = await fetch(`${window.API_BASE_URL}/api/admin/email/send-all-email`, {
        method: "POST",
        headers: {
      ...
      statusMsg.innerHTML = '<span class="error" style="color: #ff4d4d;">❌ Server se connect nahi ho paya! URL ya CORS check karo.</span>';
    } finally {
